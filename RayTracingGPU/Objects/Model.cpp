...
template <>
struct std::hash<Objects::Vertex> final
{
    size_t operator()(Objects::Vertex const& vertex) const noexcept
    {
...
private:
    static size_t Combine(const size_t hash0, const size_t hash1)
    {
...
Model Model::LoadModel(const std::string& filename)
{
    ...
    {
        VK_THROW("failed to load model '" + filename + "':\n" + objReader.Error());
    }
    ...
    if(objAttrib.normals.empty())
    {
        ...
        for(size_t i = 0; i < indices.size(); i += 3)
        {
            const auto normal = normalize(cross(
                glm::vec3(vertices[indices[i + 1]].Position) - glm::vec3(vertices[indices[i]].Position),
                glm::vec3(vertices[indices[i + 2]].Position) - glm::vec3(vertices[indices[i]].Position)));
    ...
    auto modelName = std::filesystem::path(filename).filename().string();
    LOG_INFO("Model: '{}' (vertices: {}, unique vertices: {}, materials {}", modelName, objAttrib.vertices.size(), uniqueVertices.size(), materials.size());
    LOG_INFO("Model: '{}' loaded in {:.2f} seconds", modelName, elapsed);
...
Model Model::CreateBox(const glm::vec3& p0, const glm::vec3& p1, const Render::Material& material)
{
    std::vector vertices =
        {
    ...
    return {std::move(vertices), std::move(indices), std::vector{material}, nullptr};
}

Model Model::CreateSphere(const glm::vec3& center, const float radius, const Render::Material& material, const bool isProcedural)
{
    ...
    return {std::move(vertices), std::move(indices), std::vector{material}, isProcedural ? new Sphere(center, radius) : nullptr};
}

void Model::SetMaterial(const Render::Material& material)
{
    if(m_Materials.size() != 1)
    {
        VK_THROW("cannot change material on a multi-material model");
    }
...
Model::Model(std::vector<Vertex>&& vertices, std::vector<uint32_t>&& indices, std::vector<Render::Material>&& materials, const class Procedural* procedural)
