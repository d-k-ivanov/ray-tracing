    ...
    , m_Device(device)
{
    ...
    {
        VK_THROW("empty swap chain support");
    }
    ...
    VK_CHECK(vkCreateSwapchainKHR(device.Handle(), &createInfo, nullptr, &m_SwapChain));

    m_MinImageCount = std::max(2u, details.Capabilities.minImageCount);
    m_PresentMode   = actualPresentMode;
    m_Format        = surfaceFormat.format;
    m_Extent        = extent;
    m_Images        = GetEnumerateVector(m_Device.Handle(), m_SwapChain, vkGetSwapchainImagesKHR);
    m_ImageViews.reserve(m_Images.size());

    for(const auto image : m_Images)
    {
        m_ImageViews.push_back(std::make_unique<ImageView>(device, image, m_Format, VK_IMAGE_ASPECT_COLOR_BIT));
    }
    ...
    {
        debugUtils.SetObjectName(m_Images[i], ("Swapchain Image #" + std::to_string(i)).c_str());
        debugUtils.SetObjectName(m_ImageViews[i]->Handle(), ("Swapchain ImageView #" + std::to_string(i)).c_str());
    }
}
...
SwapChain::SupportDetails SwapChain::QuerySwapChainSupport(const VkPhysicalDevice physicalDevice, const VkSurfaceKHR surface)
{
...
VkSurfaceFormatKHR SwapChain::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats)
{
    ...
    VK_THROW("found no suitable surface format");
}
